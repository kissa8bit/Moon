...
public:
    ...
    public:
        ...
        __host__ __device__ Triangle& operator*() const { return (*ptr)(); }
    ...
    __host__ __device__ virtual void add(const Triangle* objects, size_t size = 1) = 0;

    __host__ __device__ virtual const Triangle& operator[](uint32_t i) const = 0;
    __host__ __device__ virtual size_t size() const { return container_size; }

    static void destroy(HitableContainer* dpointer);
    static void add(HitableContainer* dpointer, const std::vector<Triangle>& objects);
};
