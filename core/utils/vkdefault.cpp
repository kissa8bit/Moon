...
vkDefault::Device::~Device() {
    if (descriptor) vkDestroyDevice(release(descriptor), nullptr);
}

vkDefault::Device::Device(Device&& other) noexcept {
    swap(other);
}

vkDefault::Device& vkDefault::Device::operator=(Device&& other) noexcept {
    swap(other);
    return *this;
}

void vkDefault::Device::swap(Device& other) noexcept {
    std::swap(physicalDevice, other.physicalDevice);
    std::swap(descriptor, other.descriptor);
    std::swap(queueMap, other.queueMap);
}

vkDefault::Device::Device(VkPhysicalDevice physicalDevice, const PhysicalDeviceProperties& properties, const QueueFamilies& queueFamilies, const QueueRequest& queueRequest)
...
vkDefault::Device::operator const VkDevice& () const {
    return descriptor;
}
...
VkQueue vkDefault::Device::operator()(QueueFamilyIndex familyIndex, QueueIndex queueIndex) const {
