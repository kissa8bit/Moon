...
layout(set = 0, binding = 3) uniform sampler2D bbSampler;
...
void main() {
    ...
    const vec4 baseColor = texture(Sampler, fragTexCoord);
    const vec4 blurColor = texture(blurSampler, fragTexCoord);
